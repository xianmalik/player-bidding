...
export default function DraftTool() {
  ...
  const setCurrentGame = useCallback(
    (updater) => {
      setGames((prev) => {
        ...
        next[currentGameIndex] = typeof updater === "function" ? updater(cur) : updater;
        return next;
      });
    },
    [currentGameIndex]
  ...
  const selectChamp = useCallback(
    (champKey, side = currentSide, index = currentSelection) => {
  ...
  const Slot = ({ side, index, type = "pick", displayAs }) => {
    ...
    return (
      <motion.div
        ...
        onDragStart={(e) =>
          !isReadOnly && data && handleDragStart(e, { type: "slot", side, index })
        }
        onClick={() => {
          if (!isReadOnly) {
            setCurrentSide(side);
      ...
      >
        {data ? (
          <div className="relative w-full h-full">
            ...
            {!isBan && (
              <div
                className={`absolute inset-0 bg-gradient-to-t ${isBlue ? "from-blue-950/90 via-blue-900/20" : "from-red-950/90 via-red-900/20"} to-transparent flex flex-col justify-end p-3 ${!isBlue ? "items-end" : "items-start"}`}
              >
                <p
          ...
          <div className="flex items-center justify-center h-full relative overflow-hidden">
            {!isBan && (
              <div
                className={`absolute inset-0 ${isBlue ? "[background:radial-gradient(ellipse_at_bottom_right,rgba(59,130,246,0.12)_0%,transparent_70%)]" : "[background:radial-gradient(ellipse_at_bottom_right,rgba(239,68,68,0.12)_0%,transparent_70%)]"}`}
              />
            )}
          ...
          <div className="space-y-3 shrink-0">
            ...
            <div className="flex items-center justify-between w-full">
              <div className="grid grid-cols-3 gap-2">
                {[0, 1, 2].map((i) => (
                  <Slot key={i} side={leftBanSide} index={i} type="ban" displayAs="blueBan" />
                ))}
              </div>
            ...
            {[0, 1, 2].map((i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <div className="flex-1">
                  <Slot side={leftPickSide} index={i} displayAs="blue" />
                </div>
              </div>
            ))}
            ...
            <div className="flex items-center gap-1 p-1 rounded-2xl bg-white/5 border border-white/10">
              ...
              {!isReadOnly && isFearless && games.length < 5 && (
                <button
                  ...
                  title={
                    canAddNextGame ? `Add Game ${games.length + 1}` : "Add picks before proceeding"
                  }
            ...
            {!isReadOnly && (
              <div className="flex items-center gap-2">
                <button
                  ...
                  title={
                    isFearless ? "Fearless Draft ON — click to disable" : "Enable Fearless Draft"
                  }
                  ...
                  {!isReadOnly && (
                    <>
                      ...
                      >
                        ...
                        >
                          <div
                            className={`absolute top-0.5 h-3 w-3 rounded-full bg-white transition-all ${isPublic ? "left-4.5" : "left-0.5"}`}
                          />
                        </div>
                      </DropdownMenuItem>
                    </>
              ...
              <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-2 p-4">
                <AnimatePresence mode="popLayout">
                  {filteredChampions.map(([key, champ]) => {
                    const isBanned = [...selected.blueBan, ...selected.redBan].some(
                      (s) => s?.id === champ.id
                    );
                    const isPicked = [...selected.blue, ...selected.red].some(
                      (s) => s?.id === champ.id
                    );
                    ...
                    return (
                      <motion.div
                        ...
                        onDragStart={(e) =>
                          !isReadOnly && handleDragStart(e, { type: "champion", champKey: key })
                        }
        ...
        <div ref={redRef} className="md:col-span-3 order-3 md:order-3 space-y-6 flex flex-col">
          <div className="space-y-3 text-right shrink-0">
            ...
            <div className="flex items-center justify-between w-full">
              <div className="grid grid-cols-2 gap-2">
                {[3, 4].map((i) => (
                  <Slot key={i} side={rightBanSide} index={i} type="ban" displayAs="redBan" />
                ))}
              </div>
          ...
          <div className="space-y-4 flex-1">
            {[0, 1, 2].map((i) => (
              <div key={i} className="flex items-center gap-2">
                <div className="flex-1">
                  <Slot side={rightPickSide} index={i} displayAs="red" />
                </div>
